...
exports.handler = async function (event, context, callback) {  
  ...
  try {
    await axios.post('https://slack.com/api/chat.postMessage', {
      ...
      blocks: [ 
        ...data.list.map((city) => ({ type: 'section', text: { type: 'mrkdwn', text: displayText(city) } })),
        { type: 'divider' },
        { type: 'section', text: { type: 'mrkdwn', text: `*_You should probably play it safe and stay inside!_* :colin_kent:` } }
      ]
    });
  } catch (err) {
    await axios.post('https://slack.com/api/chat.postMessage', {
